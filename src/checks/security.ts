      ...
      try {
        ...
        for (const file of files) {
          ...
          if (content && secretRegex.test(content)) {
            return fail(
              this,
              `Possible secret leaked in ${file}`,
        ...
        if (usesMatch) {
          for (const use of usesMatch) {
            ...
            if (version && version.length < 40 && !/^v?\d+(\.\d+)*$/.test(version)) {
              return fail(
                this,
                `Action uses unpinned ref '${version}'`,
                'Use a version tag (e.g. @v4) or pin by full SHA hash',
                wf,
              )
            }
          }
      ...
      try {
        ...
        for (const [name, version] of Object.entries(deps)) {
          if (
            ...
            (version === '*' || version === '' || version === 'latest' || version.startsWith('>'))
          ) {
            return fail(
              this,
              `Dependency ${name} uses unsafe version range '${version}'`,
              'Use a specific version range (e.g. ^1.2.3)',
              'package.json',
            )
          }
        }
