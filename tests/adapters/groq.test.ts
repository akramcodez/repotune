...
describe('groq adapter', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    vi.spyOn(storeModule, 'getConfig').mockReturnValue({ apiKey: 'gsk-test', model: 'llama-3.3-70b-versatile', provider: 'groq' })
  })
  ...
  it('throws error if generation fails', async () => {
    ...
    await expect(groqAdapter.generate('prompt', 'context')).rejects.toThrow('Groq error: 429 - Rate limit exceeded')
  })
})
