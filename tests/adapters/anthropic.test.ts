...
describe('anthropic adapter', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    vi.spyOn(storeModule, 'getConfig').mockReturnValue({ apiKey: 'sk-ant-test', model: 'claude-haiku-4-5-20251001', provider: 'anthropic' })
  })
  ...
  it('throws error if generation fails', async () => {
    ...
    await expect(anthropicAdapter.generate('prompt', 'context')).rejects.toThrow('Anthropic error: 500 - Internal Server Error')
  })
})
